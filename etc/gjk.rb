...
def simplex_closest_point simplex
  if simplex.size == 1
    ...
    p = simplex[0]
    n = simplex[1 ... simplex.size].collect { |s| (s - p).to_a }
    t = Linalg.lstsq(NArray[*n].transpose, -NArray[*p])[0].to_a
    if t.all? { |ti| ti >= 0 } and t.inject(:+) <= 1
      return p + n.zip(t) { |ni, ti| ni * ti }.inject(:+), simplex
    else
...
gjk [Vector[1, 0], Vector[3, 1], Vector[2, -2], Vector[2, 5]]
